...
# Simuolates a nonlinear structural equation model (SEM).
def simulateNLSem(num_meas = 20, samp_size=1000, coef_low = 0.1, coef_high = 5, var_low = 0.1, var_high = 3.5, lambda_val=5):
    # Set the parameters for the simulation
    params = Parameters()
    ...
    params.set(Params.SIMULATION_PARAM1, lambda_val);
    params.set(Params.SIMULATION_PARAM2, lambda_val);
    # params.set(Params.SEED, 29493L)
    
    # Do the simulation and grab the dataset and generative graph
    sim_ = sim.NLSemSimulation(graph.RandomForward());
    sim_.createData(params, True);
    D = sim_.getDataModel(0);
    G = sim_.getTrueGraph(0);
